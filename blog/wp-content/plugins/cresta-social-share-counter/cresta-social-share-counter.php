...
function register_social_button_setting()
{
	...
	if ( get_option( 'cresta_social_shares_selected_page' ) === false ) {
	...
	if (get_option( 'cresta_social_shares_total_text' ) === false ) {
	...
	if (get_option( 'cresta_social_shares_enable_samecolors' ) === false ) {
	...
	if (get_option( 'cresta_social_shares_button_hide_show' ) === false ) {
...
function add_social_button_in_content() {
	...
	if(in_array('print',$buttons)) {
		$allButtonsSelected .= '<div class="sbutton '. $crestaShadow .' print-cresta-share" id="print-cresta-c"><a rel="nofollow" href="#" title="Print this page" onclick="window.print();"><i class="cs c-icon-cresta-print"></i></a></div>';
	}
...
echo '<!--www.crestaproject.com Social Button Floating Start--><div id="crestashareicon" class="cresta-share-icon '.$crestaSame.' '.$button_style .' '; if($show_count == 1) { echo 'show-count-active'; } echo'">';
if($button_hide_show == 1) {
	echo '<div class="cresta-the-button"><i class="c-icon-cresta-minus"></i></div>';
}

if(in_array('facebook',$buttons)) {
	echo '<div class="sbutton '. $crestaShadow .' facebook-cresta-share" id="facebook-cresta"><a rel="nofollow" href="http://www.facebook.com/sharer.php?u='. urlencode(get_permalink( $post->ID )) .'&amp;t='. htmlspecialchars(urlencode(html_entity_decode(get_the_title( $post->ID ), ENT_COMPAT, 'UTF-8')), ENT_COMPAT, 'UTF-8') .'" title="Share to Facebook" onclick="window.open(this.href,\'targetWindow\',\'toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=700,height=450\');return false;"><i class="cs c-icon-cresta-facebook"></i></a>'; if($show_count == 1) { echo '<span class="cresta-the-count" id="facebook-count"><i class="cs c-icon-cresta-spinner animate-spin"></i></span>'; } echo '</div>';
}
if(in_array('tweet',$buttons)) {
	echo '<div class="sbutton '. $crestaShadow .' twitter-cresta-share" id="twitter-cresta"><a rel="nofollow" href="http://twitter.com/share?text='. htmlspecialchars(urlencode(html_entity_decode(the_title_attribute( array( 'echo' => 0, 'post' => $post->ID ) ), ENT_COMPAT, 'UTF-8')), ENT_COMPAT, 'UTF-8') .'&amp;url='. urlencode(get_permalink( $post->ID )) .''; if($cresta_twitter_username) { echo '&amp;via=' . $cresta_twitter_username . ''; } echo '" title="Share to Twitter" onclick="window.open(this.href,\'targetWindow\',\'toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=700,height=450\');return false;"><i class="cs c-icon-cresta-twitter"></i></a>'; if($show_count == 1) { echo '<span class="cresta-the-count" id="twitter-count"><i class="cs c-icon-cresta-spinner animate-spin"></i></span>'; } echo '</div>';
}

if(in_array('gplus',$buttons)) {
	echo '<div class="sbutton '. $crestaShadow .' googleplus-cresta-share" id="googleplus-cresta"><a rel="nofollow" href="https://plus.google.com/share?url='. urlencode(get_permalink( $post->ID )) .'" title="Share to Google Plus" onclick="window.open(this.href,\'targetWindow\',\'toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=700,height=450\');return false;"><i class="cs c-icon-cresta-gplus"></i></a>'; if($show_count == 1) { echo '<span class="cresta-the-count" id="googleplus-count"><i class="cs c-icon-cresta-spinner animate-spin"></i></span>'; } echo '</div>';
}

if(in_array('linkedin',$buttons)) {
	echo '<div class="sbutton '. $crestaShadow .' linkedin-cresta-share" id="linkedin-cresta"><a rel="nofollow" href="http://www.linkedin.com/shareArticle?mini=true&amp;url='. urlencode(get_permalink( $post->ID )) .'&amp;title='. htmlspecialchars(urlencode(html_entity_decode(get_the_title( $post->ID ), ENT_COMPAT, 'UTF-8')), ENT_COMPAT, 'UTF-8') .'&amp;source='. esc_url( home_url( '/' )) .'" title="Share to LinkedIn" onclick="window.open(this.href,\'targetWindow\',\'toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=700,height=450\');return false;"><i class="cs c-icon-cresta-linkedin"></i></a>'; if($show_count == 1) { echo '<span class="cresta-the-count" id="linkedin-count"><i class="cs c-icon-cresta-spinner animate-spin"></i></span>'; } echo '</div>';
}

if(in_array('pinterest',$buttons)) {
	echo '<div class="sbutton '. $crestaShadow .' pinterest-cresta-share" id="pinterest-cresta"><a rel="nofollow" href="http://pinterest.com/pin/create/bookmarklet/?url='.urlencode(get_permalink( $post->ID )) .'&amp;media='. $pinImage .'&amp;description='. htmlspecialchars(urlencode(html_entity_decode(get_the_title( $post->ID ), ENT_COMPAT, 'UTF-8')), ENT_COMPAT, 'UTF-8') .'" title="Share to Pinterest" onclick="window.open(this.href,\'targetWindow\',\'toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=700,height=450\');return false;"><i class="cs c-icon-cresta-pinterest"></i></a>'; if($show_count == 1) { echo '<span class="cresta-the-count" id="pinterest-count"><i class="cs c-icon-cresta-spinner animate-spin"></i></span>'; } echo '</div>';
}

if(in_array('print',$buttons)) {
	echo '<div class="sbutton '. $crestaShadow .' print-cresta-share" id="print-cresta"><a rel="nofollow" href="#" title="Print this page" onclick="window.print();"><i class="cs c-icon-cresta-print"></i></a></div>';
}
...
<div id="poststuff">
 ...
 <form name="social_buttons" method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">
		...
		<li class="list-group-item">
			<label><input type="checkbox" <?php if(in_array('print',$buttons)) { echo 'checked="checked"'; }?> name="socialbuttons[]" value="print"/>Print Button</label>
		</li>	
		<li class="list-group-item">
			<label class="crestaDisabled"><input type="checkbox" name="crestaForPRO" disabled />Stumbleupon <span><?php _e('PRO version', 'crestassc'); ?></span></label>
		</li>
		<li class="list-group-item">
			<label class="crestaDisabled"><input type="checkbox" name="crestaForPRO" disabled />Buffer <span><?php _e('PRO version', 'crestassc'); ?></span></label>
		</li>
		<li class="list-group-item">
			<label class="crestaDisabled"><input type="checkbox" name="crestaForPRO" disabled />Reddit <span><?php _e('PRO version', 'crestassc'); ?></span></label>
		</li>
		<li class="list-group-item">
			<label class="crestaDisabled"><input type="checkbox" name="crestaForPRO" disabled />VK.com <span><?php _e('PRO version', 'crestassc'); ?></span></label>
		</li>
		<li class="list-group-item">
			<label class="crestaDisabled"><input type="checkbox" name="crestaForPRO" disabled />WhatsApp Button <i><?php _e('(only visible on smartphones Android and iPhone)', 'crestassc'); ?></i> <span><?php _e('PRO version', 'crestassc'); ?></span></label>
		</li>	
		<li class="list-group-item">
			<label class="crestaDisabled"><input type="checkbox" name="crestaForPRO" disabled />Email Share Button <span><?php _e('PRO version', 'crestassc'); ?></span></label>
		</li>	
		</ul>
				<ul class="list-group">
		...
		<li class="list-group-item">
			...
			<label>
				<input type="radio" name="stylebutton" <?php if(get_option('cresta_social_shares_style') == "seventeenth_style") { echo 'checked="checked"'; }?> value="seventeenth_style" >
				<img src="<?php echo plugins_url( '/images/cresta-social-share-counter-style-17.png' , __FILE__ ); ?>">
			</label>
		</li>
		</ul>
		<ul class="list-group">
		...
		<li class="list-group-item">
			<label><input type="checkbox" id="chkanim" name="enablesamecolors"  <?php if(get_option('cresta_social_shares_enable_samecolors') == "1") { echo 'checked="checked"'; }?> /><?php _e('Social Counter Box: Use the same colors of social network', 'crestassc'); ?></label>
		</li>
		<li class="list-group-item">
			...
			<small><?php _e('Visible only on floating buttons, buy the PRO version to use it in the content', 'crestassc'); ?></small>
		</li>
		<li class="list-group-item crestachoosetoshow">
			<label><input type="checkbox" id="chksocialtotal" name="showthetotal" class="crestashowsocialtotal <?php if(get_option('cresta_social_shares_show_total') == "1") { echo 'active'; }?>"  <?php if(get_option('cresta_social_shares_show_total') == "1") { echo 'checked="checked"'; }?> /><?php _e('Show Total Shares', 'crestassc'); ?></label>
		</li>
		<li class="list-group-item crestachoosetoshow crestachoosetotalshares">
			<label><?php _e('Total Shares Text:', 'crestassc'); ?> <input type="text" name="cresta-total-text" value="<?php echo get_option('cresta_social_shares_total_text');?>"/></label>
		</li>
		</ul>
		<ul class="list-group">
		...
		<li class="list-group-item">
			<label><input type="checkbox" id="chkhideshow" name="buttonhideshow"  <?php if(get_option('cresta_social_shares_button_hide_show') == "1") { echo 'checked="checked"'; }?> /><?php _e('Enable button to hide/show the floating buttons', 'crestassc'); ?></label>
		</li>
		<li class="list-group-item">
		...
		<?php
			...
			foreach ( $post_types  as $post_type ) { 
				...
				?>
					<li class="list-group-item">
						<label><input type="checkbox" <?php if(in_array( $post_type ,$show_on)) { echo 'checked="checked"'; }?> name="socialpages[]" value="<?php echo $post_type ?>"/><?php echo $post_type_name->labels->singular_name ?></label>
					</li>
				<?php
			}
		?>
...
function cresta_update_social_share_option() {
	if(isset($_POST['social_buttons_hidden'])) {
		if(isset($_POST['socialbuttons'])) {
		
			if($_POST['socialbuttons'] != '') {
				$buttons  = implode(',',$_POST['socialbuttons']);
				update_option('selected_button',$buttons);
				
			} if($_POST['socialpages'] != '') {
				$show_on  = implode(',',$_POST['socialpages']);
				update_option('cresta_social_shares_selected_page',$show_on);
			}
			
			update_option('cresta_social_shares_float',$_POST['floating']);
			update_option('cresta_social_shares_float_buttons', $_POST['floatingbuttons']);
			update_option('cresta_social_shares_style',$_POST['stylebutton']);
			update_option('cresta_social_shares_position_top',$_POST['position-top']);
			update_option('cresta_social_shares_position_left',$_POST['position-left']);
			update_option('cresta_social_shares_z_index',$_POST['stack-zindex']);
			update_option('cresta_social_shares_total_text', $_POST['cresta-total-text']);
			update_option('cresta_social_shares_twitter_username', $_POST['cresta-twitter-username']);
			
			if(isset($_POST['showthecounter'])) {
				update_option('cresta_social_shares_show_counter','1');
			} else {
				update_option('cresta_social_shares_show_counter','0');
			}
			
			if(isset($_POST['showthetotal'])) {
